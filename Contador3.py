...
def Leer():
	...
	global timestampOFF
	timeON = datetime.now()
	#area = [Top_left], [Top_Right], [bottom_Right], []
	area_pts2 = np.array([[100,180], [500,180], [500,400], [100,400]])
	...
	while(True):
		...
		imAux2 = cv2.drawContours(imAux, [area_pts2], -1, (255), -1)
		...
		fgmask2 = cv2.morphologyEx(gray, cv2.MORPH_CLOSE,  kernel)
		fgmask2 = object_detector.apply(image_area2)
	...
	#delay = (timestampOFF - timestampON)
			...
			cv2.putText(fgmask2,  str("ON:") + str(ON_formatted), (110,120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255 ), 1)
			#cv2.putText(fgmask2,  str("ON:") + str(ON_formatted), (110,120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255 ), 1)
			#cv2.putText(fgmask2, str("OFF:") + str(OFF_formatted), (110,140), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255 ), 1)
			#cv2.putText(fgmask2, str("DLY: ") + str(delay.seconds)+ ":" + str(delay.microseconds) , (110,160), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255 , 255 ), 1)
   
			cv2.putText(fgmask2, str("fps: ") + str(diff) , (250,120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255 ), 1)
			#cv2.putText(frGray, str("DELAY: ") + str(delay.seconds) , (350,170), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
			# [ y1:y2 , x1:x2 ]
			
			#cv2.imwrite(Datos+'/{}.jpg'.format(ON_formatted_image + "_" + str(diff)),objeto)
   
			objeto2 = frame[100:500,100:500]
			#objeto2 = imutils.resize(objeto2,width=400)
			#cv2.imwrite(Datos+'/{}.jpg'.format(ON_formatted_image + "_" + str(diff) + str("_")),objeto2)
			#if objeto.shape != objeto2.shape:
				#objeto2 = cv2.resize(objeto2, (objeto.shape[1], objeto.shape[0]))
			#imgHor = np.hstack((objeto, objeto2))
			#combined_image = cv2.hconcat([objeto, objeto2])
			gray_frame_rgb = cv2.cvtColor(fgmask2, cv2.COLOR_GRAY2BGR)
			objeto = gray_frame_rgb[100:500,100:500]
			#objeto = imutils.resize(objeto,width=400)
   #frame_rgb = cv2.cvtColor(objeto, cv2.COLOR_GRAY2BGR)
			combined_image = np.hstack((objeto, objeto2))
			#objeto = imutils.resize(combined_image,width=800)
			#combined_frame = cv2.add(objeto, objeto2)
   #vis = np.concatenate((objeto, objeto2), axis=1)
			#vis = cv2.hconcat([objeto, objeto2])
   			# Convert the grayscale frame to a 3-channel image 
   
			# Horizontally concatenate the color and grayscale frames
			#combined_frame = np.hstack((frame, frame_rgb))
   
			cv2.imwrite(Datos+'/{}.jpg'.format(ON_formatted_image + "_" + str(diff)),combined_image)
