...
cap = cv2.VideoCapture('videos/vidrio66.mp4') 
...
while True:
  ...
  #  cv2.fillPoly(frame, [negro], color=[0,0,0])
   ...
   # result = np.zeros((360,640,3), np.uint8)
    ...
    lines = cv2.HoughLinesP(fgmask, 1 , np.pi/180, 100, minLineLength=50,maxLineGap=50)
  ...
  #  contours, hierarchy = cv2.findContours(result, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
   # cv2.drawContours(frame, contours, -1, (0, 255, 255), 3)
    ...
    cv2.putText(frame,'COUNT: %r' %counter, (10,30), font, 1, (0, 255, 0), 2)
